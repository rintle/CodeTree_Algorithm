...
def makeSegTree(n, start, end):
    if start == end:
        seg_tree[n] = (data[start],data[start])
        return seg_tree[n]
    mid = (start+end)//2
    left_min, left_max = makeSegTree(n*2, start, mid)
    right_min, right_max = makeSegTree(n*2+1, mid+1, end)
    seg_tree[n] = (min(left_min, right_min), max(left_max, right_max))
    return seg_tree[n]
makeSegTree(1, 0, N-1)
# print(seg_tree)
def findMinMax(n, start, end, left, right):
    ...
    mid = (start + end)//2
    left_min, left_max = findMinMax(n*2, start, mid, left, right)
    right_min, right_max = findMinMax(n*2+1, mid+1, end, left, right)
    return (min(left_min, right_min), max(left_max, right_max))

for _ in range(M):
    a, b = map(int, input().split())
    print(*findMinMax(1, 0, N-1, a-1, b-1))
